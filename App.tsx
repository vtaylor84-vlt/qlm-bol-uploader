...
const App = () => {
  ...
  // ✅ Verified Logo URLs (Placeholders - ensure these are direct high-quality links)
  const companyLogos: Record<string, string> = {
    'GLX': 'https://raw.githubusercontent.com/vtaylor84-vlt/qlm-bol-uploader/main/public/glx-logo.png', 
    'BST': 'https://bstlogistics.com/wp-content/uploads/2021/04/BST-Logo-White.png',
  };

  const states = ['AL', 'AK', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'FL', 'GA', 'HI', 'IA', 'ID', 'IL', 'IN', 'KS', 'KY', 'LA', 'ME', 'MD', 'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ', 'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY'];

  // ✅ Memory Cleanup: Revoke Blob URLs when files are removed or component unmounts
  useEffect(() => {
    ...
    if (e.target.files) {
      ...
      filesArray.forEach((file) => {
        ...
        if (!isDuplicate) {
          newFiles.push({ 
            file, 
            preview: URL.createObjectURL(file), // ✅ Creates a valid temporary browser URL
  ...
  return (
    <div className="app-container space-y-6 pb-20">
      ...
      <div className="bg-[#0a0a0a] border border-zinc-800 rounded-lg p-5 shadow-2xl space-y-6">
        ...
        {/* ✅ Logo Display Area */}
        <div className="flex justify-center h-20 items-center border-b border-zinc-800 pb-4 overflow-hidden">
          {company && companyLogos[company] ? (
            <img 
              src={companyLogos[company]} 
              alt={`${company} Logo`} 
              className="max-h-full w-auto object-contain"
              onError={(e) => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/150?text=Logo+Missing'; }}
            />
          ) : (
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col">
            ...
            <input type="text" placeholder="Enter your name" className="bg-[#111] border border-zinc-700 p-2 rounded text-white text-sm outline-none" value={driverName} onChange={(e) => setDriverName(e.target.value)} />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#111] border border-zinc-800 p-4 rounded-md space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-[11px] font-bold text-white uppercase tracking-tighter">BOL / POD Uploads<span className="required-asterisk">*</span></span>
            ...
            <div className="border border-dashed border-zinc-700 p-6 rounded text-center cursor-pointer hover:bg-zinc-900 transition" onClick={() => fileInputRef.current?.click()}>
              <p className="text-white text-[10px] font-bold uppercase mb-4">Tap to open camera or upload files</p>
            ...
            {/* ✅ Fixed Preview Gallery: ensuring images display correctly */}
              ...
              <div className="grid grid-cols-3 gap-2 mt-4">
                {uploadedFiles.map((item) => (
                  <div key={item.id} className="relative aspect-square border border-zinc-700 rounded bg-zinc-900 overflow-hidden">
                    ...
                    />
                    <button onClick={() => removeFile(item.id)} className="absolute top-0 right-0 bg-red-600 text-white text-[10px] p-1 px-2 rounded-bl z-10">X</button>
                  </div>
                ))}
              </div>
            )}
