...
const App = () => {
  ...
  useEffect(() => {
    ...
    // Ensure URLs are cleared on unmount
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      const filesArray = Array.from(e.target.files).map(file => ({
        file,
        preview: URL.createObjectURL(file), // ✅ FORCED FIX: Direct blob creation
  ...
  const tacticalInput = `w-full bg-black border border-zinc-800 p-3 text-xs rounded-lg outline-none transition-all duration-300 text-white placeholder-zinc-700 focus:border-zinc-500 appearance-none`;
  ...
  if (isLocked) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6 font-orbitron overflow-hidden relative">
        <div className="absolute inset-0 opacity-20 bg-[linear-gradient(#111_1px,transparent_1px),linear-gradient(90deg,#111_1px,transparent_1px)] bg-[size:40px_40px]"></div>
        <div className="z-10 flex flex-col md:flex-row items-center gap-12 max-w-2xl w-full p-8 border border-zinc-900 bg-zinc-950/50 rounded-3xl backdrop-blur-md">
          <div className="flex flex-col items-center space-y-6">
            ...
            <button 
              ...
              onTouchStart={startAuthSequence} 
              className={`relative w-32 h-32 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${isAuthenticating ? 'border-cyan-500 shadow-[0_0_50px_rgba(6,182,212,0.5)] scale-105' : 'border-zinc-800 hover:border-zinc-600'}`}
            >
               <svg className={`w-16 h-16 ${isAuthenticating ? 'text-cyan-500' : 'text-zinc-700'} transition-colors duration-500`} fill="currentColor" viewBox="0 0 24 24">
            ...
            <p className={`text-[8px] uppercase tracking-[0.3em] transition-all ${isAuthenticating ? 'text-cyan-400' : 'text-zinc-600'}`}>
              {isAuthenticating ? 'VERIFYING BIOMETRICS...' : 'HOLD FINGER TO LOGIN'}
            </p>
          </div>
          <div className="flex-1 w-full space-y-4 font-mono text-[10px] h-32 flex flex-col justify-center border-l border-zinc-900 pl-8">
            <div className={authStage >= 1 ? 'text-cyan-500' : 'text-zinc-800'}>{"> KERNEL INIT [OK]"}</div>
            <div className={authStage >= 2 ? 'text-cyan-500' : 'text-zinc-800'}>{"> PROXY_v7.0 READY"}</div>
            <div className={authStage >= 3 ? 'text-cyan-500' : 'text-zinc-800'}>{"> GEO_IP LOGGED"}</div>
            <div className={authStage >= 4 ? 'text-green-500 font-bold' : 'text-zinc-800'}>{authStage >= 4 ? "> ACCESS AUTHORIZED" : "> AWAITING INPUT..."}</div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#020202] text-white pb-20 relative font-orbitron overflow-hidden">
      ...
      <div className="relative z-10 p-5 max-w-xl mx-auto space-y-8">
          ...
          <div className="flex items-center gap-4">
            {isGLX ? (
              <div className="w-14 h-14 bg-green-500 rounded-lg flex flex-col items-center justify-center font-black text-black leading-none shadow-[0_0_20px_rgba(34,197,94,0.4)]">
                <span className="text-lg">GLX</span><span className="text-[6px] tracking-widest mt-1">LOGISTICS</span>
              </div>
            ) : isBST ? (
              <div className="w-14 h-14 bg-blue-500 rounded-lg flex flex-col items-center justify-center font-black text-white leading-none shadow-[0_0_20px_rgba(59,130,246,0.4)]">
                <span className="text-lg">BST</span><span className="text-[6px] tracking-widest mt-1">EXPEDITE</span>
              </div>
            ) : (
            ...
            <div className="space-y-1">
               <h1 className={`text-xl font-black tracking-widest uppercase ${brandColor}`}>Terminal v7.0</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className={`flex flex-col p-3 rounded-xl border border-zinc-900 transition-all ${company ? brandBg : ''}`}>
            ...
            <select className={tacticalInput} value={company} onChange={(e) => setCompany(e.target.value)}>
             ...
             <div className="grid grid-cols-3 gap-4 mt-6">
               {uploadedFiles.map(f => (
                 <div key={f.id} className="relative aspect-square rounded-xl border border-zinc-800 overflow-hidden group">
                   ...
                   <button onClick={() => setUploadedFiles(prev => prev.filter(item => item.id !== f.id))} className="absolute top-1 right-1 bg-red-600 text-white text-[8px] p-1.5 rounded-full z-30 opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                 </div>
               ))}
             </div>
           )}
        ...
        <div className="bg-zinc-950/40 border border-zinc-900 p-6 rounded-2xl space-y-4">
          ...
          <button onClick={() => freightInputRef.current?.click()} className="w-full bg-zinc-900/30 border border-zinc-800 p-5 rounded-xl text-[10px] font-black uppercase tracking-widest text-zinc-500 flex flex-col items-center gap-3 hover:text-white transition-all">
             <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 13l-3 3m0 0l-3-3m3 3V8m0 13a9 9 0 110-18 9 9 0 010 18z"/></svg>
             Photos of freight loaded on trailer
          </button>
        </div>

        <button onClick={handleSubmit} className={`w-full py-6 rounded-xl font-black text-xs uppercase tracking-[0.5em] transition-all relative overflow-hidden ${shake ? 'animate-shake bg-red-600' : isFormComplete ? (isGLX ? 'bg-green-600 shadow-[0_0_40px_rgba(34,197,94,0.3)]' : 'bg-blue-600 shadow-[0_0_40px_rgba(59,130,246,0.3)]') : 'bg-zinc-900 text-zinc-700'}`}>
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/98 z-50 flex flex-col items-center justify-center p-8 backdrop-blur-3xl animate-in zoom-in-95 duration-500">
           ...
           <h2 className="text-2xl font-black uppercase tracking-[0.4em] mb-4 text-white text-center">Transmission Verified</h2>
           <button onClick={() => window.location.reload()} className={`w-full max-w-xs py-4 border-2 text-[11px] font-black uppercase tracking-[0.3em] ${brandColor}`}>Initialize New Session</button>
        </div>
      )}
      ...
      <style>{`
        ...
        .animate-scan-limited { animation: scan-limited 2s linear 3; }
      ...
      <input type="file" ref={freightInputRef} className="hidden" multiple accept="image/*" onChange={handleFile} />
    </div>
  );
};
