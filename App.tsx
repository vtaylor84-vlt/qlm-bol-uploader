...
const App = () => {
  ...
  // ✅ AUTHENTICATION: Sequential Logic Fix
  const handleAuth = () => {
  ...
  // ✅ IMAGE ENGINE: Direct Preview Generation
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-6 font-orbitron overflow-hidden relative">
        ...
        <div className="z-10 flex flex-col md:flex-row items-center gap-12 max-w-2xl w-full p-10 border border-zinc-900 bg-zinc-950/80 rounded-3xl shadow-2xl backdrop-blur-lg">
          ...
          <div className="flex-1 w-full space-y-4 font-mono text-[10px] h-32 flex flex-col justify-center border-l border-zinc-900 pl-8">
            <div className={authStage >= 1 ? 'text-cyan-500' : 'text-zinc-900'}>{"> SEC_PROTOCOL_v8.5"}</div>
            <div className={authStage >= 2 ? 'text-cyan-500' : 'text-zinc-900'}>{"> GEO_LOC_IP_LOGGED"}</div>
            <div className={authStage >= 3 ? 'text-cyan-500' : 'text-zinc-900'}>{"> UPLINK_ENCRYPTION_ACTIVE"}</div>
            <div className={authStage >= 4 ? 'text-green-500 font-bold' : 'text-zinc-900'}>{authStage >= 4 ? "> SESSION_AUTHORIZED" : "> AWAITING_BIOMETRIC"}</div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#020202] text-white pb-24 relative font-orbitron overflow-hidden">
      ...
      <div className="relative z-10 p-5 max-w-xl mx-auto space-y-10">
        <header className="flex justify-between items-center border-b border-zinc-900 pb-8">
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
               <h1 className={`text-xl font-black tracking-widest uppercase ${brandColor}`}>Terminal v8.5</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className={`flex flex-col p-4 rounded-xl border border-zinc-900 ${driverName ? 'bg-zinc-900/40' : ''}`}>
            <label className={labelStyle}>Operator Full Name</label>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className={`flex flex-col p-4 rounded-xl border border-zinc-900 ${loadNum ? 'bg-zinc-900/40' : ''}`}>
            ...
            <input type="text" placeholder="LOAD ID" className={inputStyle} value={loadNum} onChange={(e) => setLoadNum(e.target.value)} />
          </div>
          <div className={`flex flex-col p-4 rounded-xl border border-zinc-900 ${bolNum ? 'bg-zinc-900/40' : ''}`}>
            ...
            <input type="text" placeholder="BOL ID" className={inputStyle} value={bolNum} onChange={(e) => setBolNum(e.target.value)} />
          </div>
        </div>

        {/* ✅ RESTORED: ROUTE NAVIGATION GRID (City & State) */}
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
             <div className={`col-span-2 flex flex-col p-4 rounded-xl border border-zinc-900 ${puCity ? 'bg-zinc-900/40' : ''}`}>
               ...
               <input type="text" placeholder="CITY NAME" className={inputStyle} value={puCity} onChange={(e) => setPuCity(e.target.value)} />
             </div>
          ...
          <div className="grid grid-cols-3 gap-4">
             <div className={`col-span-2 flex flex-col p-4 rounded-xl border border-zinc-900 ${delCity ? 'bg-zinc-900/40' : ''}`}>
               ...
               <input type="text" placeholder="CITY NAME" className={inputStyle} value={delCity} onChange={(e) => setDelCity(e.target.value)} />
             </div>
        ...
        <div className={`bg-zinc-950/40 border-2 border-dashed p-8 rounded-2xl text-center transition-all ${uploadedFiles.length > 0 ? (isGLX ? 'border-green-500/50' : 'border-blue-500/50') : 'border-zinc-800'}`}>
           <h2 className={`text-[10px] font-black uppercase tracking-widest mb-8 ${brandColor}`}>Imaging Terminal</h2>
           ...
           <div className="flex justify-center gap-12 mb-10">
              <button onClick={() => fileInputRef.current?.click()} className={`${brandColor} text-[10px] font-black uppercase flex flex-col items-center gap-3`}><svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg> Gallery</button>
              <button onClick={() => cameraInputRef.current?.click()} className={`${brandColor} text-[10px] font-black uppercase flex flex-col items-center gap-3`}><svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg> Camera</button>
           </div>
             ...
             <div className="grid grid-cols-3 gap-5 mt-10">
               {uploadedFiles.map(f => (
                 <div key={f.id} className="relative aspect-square rounded-2xl border border-zinc-800 overflow-hidden group">
                   ...
                   <div className={`absolute top-0 left-0 w-full h-[2px] animate-scan-limited ${isGLX ? 'bg-green-500' : 'bg-cyan-500'}`}></div>
        ...
        {/* ✅ STREAMLINED TRAILER MODULE */}
        <div className="bg-zinc-950/40 border border-zinc-900 p-8 rounded-3xl space-y-6">
          <div className="flex justify-between items-center"><h2 className={`text-[10px] font-black uppercase tracking-widest ${brandColor}`}>Trailer Visuals</h2><span className="text-[7px] text-zinc-600 uppercase">Optional Module</span></div>
        ...
        <button onClick={triggerUplink} className={`w-full py-8 rounded-2xl font-black text-[11px] uppercase tracking-[0.6em] transition-all relative overflow-hidden ${shake ? 'animate-shake bg-red-600' : isFormComplete ? (isGLX ? 'bg-green-600 shadow-[0_0_50px_rgba(34,197,94,0.3)]' : 'bg-blue-600 shadow-[0_0_50px_rgba(59,130,246,0.3)]') : 'bg-zinc-900 text-zinc-700 border border-zinc-800'}`}>
          ...
          <span className="relative z-10">{isSubmitting ? 'TRANSMITTING...' : 'Execute Final Transmission'}</span>
        </button>
      </div>
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/98 z-50 flex flex-col items-center justify-center p-10 backdrop-blur-3xl animate-in zoom-in-95 duration-700">
           ...
           <h2 className="text-3xl font-black uppercase tracking-[0.5em] mb-4 text-white text-center">Transmission Success</h2>
           <button onClick={() => window.location.reload()} className={`w-full max-w-sm py-5 border-2 text-[12px] font-black uppercase tracking-[0.4em] ${brandColor} hover:bg-white/5`}>Initialize New Protocol</button>
        </div>
      )}
