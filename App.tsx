...
const App = () => {
  ...
  const getContainerStyles = () => {
    if (isGLX) return 'border-green-500 shadow-[0_0_25px_rgba(34,197,94,0.2)]';
    if (isBST) return 'border-blue-500 shadow-[0_0_25px_rgba(59,130,246,0.2)]';
      ...
      <div className={`bg-[#0a0a0a] border rounded-lg p-5 transition-all duration-700 space-y-8 ${getContainerStyles()}`}>
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="bg-[#111] border border-zinc-800 p-4 rounded-md space-y-4 shadow-inner">
            ...
            <div className={`border border-dashed border-zinc-700 p-6 rounded text-center cursor-pointer transition-all ${isGLX ? 'hover:border-green-500' : isBST ? 'hover:border-blue-500' : 'hover:border-cyan-500'}`} onClick={() => fileInputRef.current?.click()}>
              ...
              <div className="grid grid-cols-3 gap-2 mt-4">
                {uploadedFiles.map((item) => (
                  <div key={item.id} className="relative aspect-square border border-zinc-700 rounded bg-zinc-900 overflow-hidden">
                    ...
                    <button onClick={() => removeFile(item.id)} className="absolute top-0 right-0 bg-red-600 text-white text-[10px] p-1 px-2 rounded-bl shadow-lg">X</button>
                  </div>
                ))}
              </div>
            )}
