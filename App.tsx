...
/**
 * LOGISTICS TERMINAL v6.5 - SENSORY RESTORATION
 * - Restored: CAMERA/GALLERY labels, "Complete Fields" button logic.
 * - Restored: Theme Pulse & Tactical Shake on field completion.
 * - Feature: Auto-Uppercase for industry-standard data entry.
 */
...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen bg-[#050505] text-zinc-100 pb-24 transition-all duration-500 ${shake ? 'animate-shake' : ''}`}>
      {/* Theme Pulse Overlay */}
      <div className={`fixed inset-0 pointer-events-none transition-opacity duration-500 z-50 ${pulseActive ? 'opacity-20' : 'opacity-0'}`} 
      ...
      <div className="max-w-4xl mx-auto space-y-6 px-4 relative">
        ...
        {/* DOCUMENT UPLINK */}
        <section className={`rounded-[2.5rem] p-8 border-2 transition-all duration-700 ${bolProtocol ? 'bg-black border-zinc-700 shadow-2xl' : 'bg-zinc-900/20 border-zinc-800 border-dashed'}`}>
          <div className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-10">
            <h3 className={`text-[11px] font-black uppercase tracking-[0.6em] ${themeColor}`}>[ 04 ] Document Uplink</h3>
            <div className="flex gap-4">
              <button onClick={() => {setBolProtocol('PICKUP'); triggerPulse();}} className={`px-8 py-3 text-[10px] font-black rounded-xl border-2 transition-all ${bolProtocol === 'PICKUP' ? 'bg-black text-white border-white' : 'bg-white text-zinc-500 border-zinc-200'}`}>PICKUP BOL</button>
              <button onClick={() => {setBolProtocol('DELIVERY'); triggerPulse();}} className={`px-8 py-3 text-[10px] font-black rounded-xl border-2 transition-all ${bolProtocol === 'DELIVERY' ? 'bg-black text-white border-white' : 'bg-white text-zinc-500 border-zinc-200'}`}>DELIVERY BOL</button>
            </div>
          </div>
          <div className="flex justify-center gap-16 py-6">
            <button onClick={() => cameraInputRef.current?.click()} className="flex flex-col items-center gap-4 group">
              <div className="w-24 h-24 rounded-2xl bg-zinc-800 flex items-center justify-center text-4xl border border-zinc-700 group-hover:bg-white transition-all">📸</div>
        ...
        <button 
          ...
          className={`w-full py-10 rounded-[2.5rem] font-black uppercase tracking-[1em] transition-all duration-1000 ${
            isReady ? 'bg-white text-black shadow-2xl hover:scale-[1.01]' : 'bg-zinc-900 text-zinc-700 opacity-50'
          }`}>
          {isSubmitting ? 'SECURE UPLOAD...' : isReady ? 'SUBMIT DOCUMENTS' : 'COMPLETE FIELDS'}
        </button>
      </div>

      {showSuccess && (
        <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center p-6">
           <div className="w-32 h-32 rounded-full border-4 flex items-center justify-center text-5xl mb-8 animate-bounce" style={{ borderColor: themeHex }}>✓</div>
           <h2 className="text-4xl font-black italic uppercase tracking-widest text-white">Verified</h2>
           <button onClick={() => window.location.reload()} className="mt-10 text-zinc-600 uppercase text-xs font-black tracking-widest">New Session</button>
        </div>
      )}
