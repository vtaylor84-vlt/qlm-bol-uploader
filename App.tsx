...
/**
 * DOCUMENT TERMINAL v3.1 - FINAL PRODUCTION
 * SENIOR GRADE: Multi-Carrier, Auto-Compression, Haptics, Solar Mode, Google Uplink.
 */
  ...
  return (
    <div className={`min-h-screen transition-colors duration-1000 ${solarMode ? 'bg-white text-black' : 'bg-[#020202] text-zinc-100'} pb-24 font-sans ${shake ? 'animate-shake' : ''}`}>
      ...
      <header className="max-w-4xl mx-auto pt-10 px-4 mb-12 relative z-10">
        ...
        <div className={`w-full min-h-[220px] rounded-[3.5rem] border-2 transition-all duration-1000 flex items-center justify-center ${company ? 'bg-black shadow-2xl' : 'bg-zinc-900/50 border-zinc-800'}`} style={{ borderColor: company ? themeHex : '' }}>{!company && <h1 className="text-5xl font-black italic tracking-tighter uppercase text-zinc-700">DOCUMENT UPLOADER</h1>}{company === 'GLX' && <GreenleafLogo />}{company === 'BST' && <BSTLogo />}</div>
      </header>
      <div className="max-w-4xl mx-auto space-y-8 px-4 relative z-10">
        ...
        <section className={`bg-zinc-900/40 border-2 rounded-[2.5rem] p-8 shadow-2xl ${solarMode ? 'bg-zinc-50 border-zinc-200' : ''}`} style={{ borderColor: (puCity && delCity) ? themeHex : '#27272a' }}>
          ...
          <div className="grid grid-cols-3 gap-6 mb-6"><input type="text" placeholder="PICKUP CITY" className={`col-span-2 p-5 rounded-2xl border-2 outline-none ${getInputStyle(!!puCity)}`} value={puCity} onChange={(e) => handleInputChange(setPuCity, e.target.value)} /><select className={`p-5 rounded-2xl border-2 outline-none ${getInputStyle(!!puState)}`} value={puState} onChange={(e) => setPuState(e.target.value)}><option value="">ST</option>{states.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
          <div className="grid grid-cols-3 gap-6"><input type="text" placeholder="DELIVERY CITY" className={`col-span-2 p-5 rounded-2xl border-2 outline-none ${getInputStyle(!!delCity)}`} value={delCity} onChange={(e) => handleInputChange(setDelCity, e.target.value)} /><select className={`p-5 rounded-2xl border-2 outline-none ${getInputStyle(!!delState)}`} value={delState} onChange={(e) => setDelState(e.target.value)}><option value="">ST</option>{states.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
        </section>
        <section className={`rounded-[2.5rem] p-8 border-2 transition-all ${bolProtocol ? (solarMode ? 'bg-zinc-100' : 'bg-black') : 'opacity-40 border-dashed'}`} style={{ borderColor: bolProtocol ? themeHex : '#27272a', backgroundColor: bolProtocol ? `${themeHex}${solarMode ? '10' : '20'}` : '' }}>
          ...
          <div className={`flex justify-center gap-16 py-6 transition-all ${bolProtocol ? 'opacity-100' : 'opacity-20 pointer-events-none'}`}><button onClick={() => cameraInputRef.current?.click()} className="flex flex-col items-center gap-4 group"><div className={`w-20 h-20 rounded-2xl flex items-center justify-center text-4xl border ${solarMode ? 'bg-white' : 'bg-zinc-800'}`}>📸</div><span className="text-[10px] font-black uppercase text-zinc-500">Camera</span></button><button onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center gap-4 group"><div className={`w-20 h-20 rounded-2xl flex items-center justify-center text-4xl border ${solarMode ? 'bg-white' : 'bg-zinc-800'}`}>📂</div><span className="text-[10px] font-black uppercase text-zinc-500">Gallery</span></button></div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">{uploadedFiles.filter(f => f.category === 'bol').map(f => (<div key={f.id} className="aspect-[3/4] rounded-2xl bg-zinc-900 overflow-hidden relative border border-zinc-800"><img src={f.preview} className="w-full h-full object-cover" /><button onClick={() => setUploadedFiles(p => p.filter(i => i.id !== f.id))} className="absolute top-2 right-2 w-7 h-7 bg-red-600 text-white rounded-full flex items-center justify-center text-xs">✕</button></div>))}</div>
        </section>
        {bolProtocol === 'PICKUP' && (<section className={`transition-all rounded-[2.5rem] p-8 border-2 ${uploadedFiles.some(f => f.category === 'freight') ? (solarMode ? 'bg-zinc-100' : 'bg-black') : 'opacity-40 border-dashed'}`} style={{ borderColor: uploadedFiles.some(f => f.category === 'freight') ? themeHex : '#27272a', backgroundColor: uploadedFiles.some(f => f.category === 'freight') ? `${themeHex}${solarMode ? '10' : '20'}` : '' }}><h3 className={`text-[11px] font-black uppercase tracking-[0.6em] mb-8 ${uploadedFiles.some(f => f.category === 'freight') ? themeColor : 'text-zinc-500'}`}>[ 05 ] Freight Photos</h3><div className="flex justify-center gap-12 py-6"><button onClick={() => freightCamRef.current?.click()} className="flex flex-col items-center gap-4 group"><div className={`w-20 h-20 rounded-2xl flex items-center justify-center text-4xl border ${solarMode ? 'bg-white' : 'bg-zinc-800'}`}>📸</div><span className="text-[10px] font-black text-zinc-500 uppercase">Camera</span></button></div><div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">{uploadedFiles.filter(f => f.category === 'freight').map(f => (<div key={f.id} className="aspect-square rounded-2xl bg-zinc-900 overflow-hidden relative border border-zinc-800"><img src={f.preview} className="w-full h-full object-cover" /><button onClick={() => setUploadedFiles(p => p.filter(i => i.id !== f.id))} className="absolute top-2 right-2 w-7 h-7 bg-red-600 text-white rounded-full flex items-center justify-center text-xs">✕</button></div>))}</div></section>)}
        <button onClick={() => { if(!isReady) { playSound(100, 'square', 0.3); triggerHaptic(100); setShake(true); setTimeout(() => setShake(false), 500); } else { playSound(600, 'sine', 0.3); triggerHaptic([30, 100]); setShowVerification(true); } }} className={`w-full py-10 rounded-[2.5rem] font-black uppercase tracking-[1.5em] transition-all duration-1000 border-[3px] border-white ${isReady ? `bg-gradient-to-r ${company === 'GLX' ? 'from-green-600 via-green-400 to-green-600' : 'from-blue-600 via-blue-400 to-blue-600'} text-black shadow-2xl` : 'bg-zinc-900 text-zinc-700 opacity-50 cursor-not-allowed'}`}><span className="relative z-10">{isReady ? 'REVIEW DOCUMENTS' : 'COMPLETE FIELDS'}</span></button>
      </div>
