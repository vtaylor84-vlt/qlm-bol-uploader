...
/**
 * LOGISTICS TERMINAL v5.2 - BLACK-OPS EDITION
 * Features: High-Contrast Neon UI, Radial Gradient Glows, Metallic SVG Logos.
 */
...
const BSTLogo = () => (
  <div className="flex flex-col items-center justify-center p-6 animate-in zoom-in duration-1000">
    <svg width="340" height="140" viewBox="0 0 400 140" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
const App: React.FC = () => {
  ...
  const themeHex = isGLX ? '#22c55e' : isBST ? '#3b82f6' : '#ffffff';
  ...
  const getInputStyles = (value: string) => {
    ...
    return `w-full p-5 rounded-2xl font-mono text-sm transition-all duration-500 border-2 outline-none
      ${isFilled 
        ...
        : 'bg-zinc-100 text-black border-zinc-200 focus:bg-white focus:border-zinc-400 focus:shadow-xl'}`;
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 pb-20 selection:bg-blue-500 selection:text-white">
      ...
      <header className="max-w-4xl mx-auto pt-10 px-4 mb-12 relative">
        <div className={`w-full min-h-[220px] rounded-[3.5rem] border-2 transition-all duration-1000 flex items-center justify-center ${
          ...
          {!company && <h1 className="text-5xl font-black italic tracking-tighter uppercase opacity-40">BOL Uplink</h1>}
          {isGLX && <GreenleafLogo />}
          {isBST && <BSTLogo />}
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto space-y-6 px-4 relative">
        {/* MODULE 01: IDENTITY */}
        <section className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 blur-[60px] group-hover:bg-blue-500/10 transition-all" />
          <h3 className={`text-[11px] font-black uppercase tracking-[0.6em] mb-8 ${themeColor}`}>[ 01 ] Carrier & Operator</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
            ...
            <input type="text" placeholder="ENTER NAME" className={getInputStyles(driverName)} value={driverName} onChange={(e) => setDriverName(e.target.value)} />
          </div>
        </section>

        {/* MODULE 02: CARGO DATA */}
        ...
        <section className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800 rounded-[2.5rem] p-8 shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-4">
              ...
              <input type="text" placeholder="PICKUP STATE" className={getInputStyles(puState)} value={puState} onChange={(e) => setPuState(e.target.value.toUpperCase())} />
            </div>
            <div className="space-y-4">
              ...
              <input type="text" placeholder="DELIVERY STATE" className={getInputStyles(delState)} value={delState} onChange={(e) => setDelState(e.target.value.toUpperCase())} />
            </div>
          </div>
        </section>
        ...
        <section className={`rounded-[2.5rem] p-8 border transition-all duration-700 ${
          bolProtocol ? 'bg-black border-zinc-700 shadow-[0_0_50px_rgba(0,0,0,0.5)]' : 'bg-zinc-900/20 border-zinc-800 border-dashed'
        }`}>
          <div className="flex justify-between items-center mb-10">
            <h3 className={`text-[11px] font-black uppercase tracking-[0.6em] ${themeColor}`}>[ 04 ] Paperwork Secure Scan</h3>
            <div className="flex gap-3">
              <button onClick={() => setBolProtocol('PICKUP')} className={`px-8 py-3 text-[10px] font-black rounded-xl border transition-all ${bolProtocol === 'PICKUP' ? 'bg-white text-black border-white' : 'border-zinc-800 text-zinc-600'}`}>PICKUP</button>
              <button onClick={() => setBolProtocol('DELIVERY')} className={`px-8 py-3 text-[10px] font-black rounded-xl border transition-all ${bolProtocol === 'DELIVERY' ? 'bg-white text-black border-white' : 'border-zinc-800 text-zinc-600'}`}>DELIVERY</button>
            </div>
          </div>
          
          <div className="flex justify-center py-10">
            <button className="w-24 h-24 rounded-full bg-zinc-800 flex items-center justify-center text-3xl hover:bg-white hover:text-black transition-all duration-500 shadow-2xl">📸</button>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {uploadedFiles.map(f => (
              <div key={f.id} className="aspect-[3/4] rounded-2xl bg-zinc-800 overflow-hidden border border-zinc-700">
        ...
        <button 
          onClick={() => { setIsSubmitting(true); setTimeout(() => setShowSuccess(true), 2500); }}
          className={`w-full py-10 rounded-[2.5rem] font-black uppercase tracking-[1.5em] transition-all duration-1000 ${
            driverName && bolProtocol ? 'bg-white text-black shadow-[0_0_60px_rgba(255,255,255,0.2)] hover:scale-[1.02]' : 'bg-zinc-900 text-zinc-800 cursor-not-allowed'
          }`}>
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center p-6 animate-in fade-in">
           <div className="w-32 h-32 rounded-full border-4 border-green-500 flex items-center justify-center text-5xl mb-12 animate-bounce">✓</div>
           <h2 className="text-4xl font-black italic uppercase tracking-[0.5em] text-white">Transmission Verified</h2>
