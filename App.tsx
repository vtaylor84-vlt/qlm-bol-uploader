        ...
        <section className={`rounded-[2.5rem] p-8 border-2 transition-all duration-700 ${section4Ready ? 'bg-black' : 'bg-zinc-900/20 border-zinc-800 border-dashed'}`} style={{ borderColor: section4Ready ? themeHex : '' }}>
          <div className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-10">
            ...
            <div className="flex gap-4">
              <button onClick={() => {setBolProtocol('PICKUP'); triggerPulse();}} className={`px-6 py-3 text-[10px] font-black rounded-xl border-2 transition-all duration-500 ${bolProtocol === 'PICKUP' ? `bg-black text-white border-[${themeHex}] shadow-lg` : 'bg-white text-zinc-500 border-zinc-200'}`}>PICKUP BOL</button>
              <button onClick={() => {setBolProtocol('DELIVERY'); triggerPulse();}} className={`px-6 py-3 text-[10px] font-black rounded-xl border-2 transition-all duration-500 ${bolProtocol === 'DELIVERY' ? `bg-black text-white border-[${themeHex}] shadow-lg` : 'bg-white text-zinc-500 border-zinc-200'}`}>DELIVERY BOL</button>
            </div>
          </div>
